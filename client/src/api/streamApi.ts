...
export const streamKeyApi = {
  async getStreamKey(userId: string) {
    const response = await api.get<ApiResponse<StreamKeyResponse>>(
      `/stream-keys/user/${userId}`,
    );
    return response.data.data;
  },

  async createStreamKey(createRequest: CreateStreamKeyRequest) {
    const response = await api.post<ApiResponse<StreamKeyResponse>>(
      `/stream-keys`,
      createRequest,
    );
    return response.data.data;
...
export const streamApi = {
  async createStream(createRequest: CreateStreamRequest) {
    const response = await api.post<ApiResponse<StreamResponse>>(
      `/streams`,
      createRequest,
      {
        headers: {
  ...
  async getStreamOnlineOfUser(userId: string) {
    const response = await api.get<ApiResponse<StreamResponse>>(
      `/streams/user/${userId}`,
    );
    return response.data.data;
  },
  ...
  async getTopStream() {
    const response =
      await api.get<ApiResponse<StreamResponse[]>>(`/streams/top`);
    return response.data.data;
  },
  async startStreamById(id: string) {
    const response = await api.patch<ApiResponse<StreamResponse>>(
      `/streams/start/${id}`,
    );
    return response.data.data;
  },
  ...
  async checkStatusLikedStream(id: string) {
    const response = await api.get<ApiResponse<LivestreamLikeStatusResponse>>(
      `/streams/like-status/${id}`,
    );
    return response.data.data;
  },
};
...
export const streamChatApi = {
  async getChats(streamId: string) {
    const response = await api.get<ApiResponse<StreamChatResponse[]>>(
      `/stream-chats/stream/${streamId}`,
    );
    return response.data.data;
  },
  async sendChat(streamId: string, userId: string, content: string) {
    const response = await api.post<ApiResponse<StreamChatResponse>>(
      `/stream-chats`,
      {
        streamId,
        userId,
